...
interface TVShowCardCompactProps {
  show: TVShow;
  priority?: boolean;
}

export function TVShowCardCompact({ show, priority = false }: TVShowCardCompactProps) {
  return (
    <Link
      href={`/tv/${show.id}`}
    ...
    >
      ...
      <div className="flex flex-1 flex-col justify-center">
        <div className="flex items-center gap-2">
          <span className="rounded-md bg-blue-500/20 px-1.5 py-0.5 text-[10px] font-bold text-blue-400">
            TV
          </span>
          <h3 className="line-clamp-1 text-sm font-semibold text-white transition-colors group-hover:text-rose-400">
            {show.name}
          </h3>
        </div>
        ...
        <div className="mt-2 flex items-center gap-1.5">
          <div className="flex items-center gap-1 rounded-md bg-yellow-500/20 px-2 py-0.5">
            ...
            <span className="text-xs font-medium text-yellow-400">{formatVoteAverage(show.vote_average)}</span>
          </div>
        </div>
      </div>
    </Link>
